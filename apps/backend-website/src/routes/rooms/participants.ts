    ...
    app
      ...
      .post('/:id/participants', async ({ headers, params, body, set }) => {
        try {
          ...
          const requesterFid = headers['x-user-privyid'] as string;

          // Authorization check - only host can add participants
          const requester = await User.findOne({ privyId: requesterFid });
          if (!requester) {
            set.status = 404;
          ...
          const user = await User.findOne({ privyId: userFid });
          if (!user) {
            set.status = 404;
...
**Authentication Required:** Yes (Farcaster JWT)
      ...
      .post('/:id/join', async ({ headers, params, set }) => {
        try {
          const userFid = headers['x-user-privyid'] as string;
